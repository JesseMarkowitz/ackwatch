...
**Closed 2026-09-05: the bundled alert tone was inaudible because there was almost nothing there.** The clip was 17 samples of 8-bit audio at 8 kHz — 2.1 milliseconds, a click rather than a tone — so the developer who eventually heard it, at a volume nobody would run a machine at, reasonably read it as a fault elsewhere. It is now a rising three-note figure of 0.72 s at 16-bit/16 kHz, synthesised by `tools/generate-alert-tone.py` and still inlined as a `data:` URI so no audio is fetched. A duration and amplitude floor is asserted in `alert-tone-source.test.ts`: crude, but exactly the property that was wrong. **Not verified by ear by the assistant** — whether it is the right sound is the developer's call.

**Closed 2026-09-05: a deployment can supply its own alert tone.** `alert-tone.wav` or `alert-tone.mp3` dropped beside `index.html` is used in place of the bundled tone, probed once per session, with no rebuild and no setting; the test control reports which sound played so a misplaced file is visible. It requires `media-src 'self'`, which is now in the qualified policy and the documented template, and the probe's two 404s on deployments without a custom tone are a named pattern in the Gate 5 console classifier rather than unexplained noise.
